...
const CreatePlatformForm: React.FC<RouteComponentProps> = (props: any) => {
  ...
  useEffect(() => {
    ...
    register('platform_img', { required: true });
  ...
  const handlePlatformImageChange = (event: any) => {
    const file = event.file;
    console.log(file);
    const reader = new FileReader();
    reader.onload = (e: any) => {
      setValue('platform_img', e.target.result);
    };
  ...
  const platfromImage = {
    name: 'file',
    onRemove: (file: any) => {
      const index = platformImage.indexOf(file);
      const newFileList = platformImage.slice();
      newFileList.splice(index, 1);
      setPlatformImage(newFileList);
    },
    beforeUpload: (file: any) => {
      setPlatformImage([...platformImage, file]);
      return false;
    },
    platformImage,
  };
  return (
    <>
      ...
      >
        ...
        <div className="create-platform-location">
          <Upload {...platfromImage} onChange={handlePlatformImageChange}>
            <Button icon={<UploadOutlined />} onChange={openUploadImage}>
              Add Platform Image
            </Button>
          </Upload>
        </div>
