...
const Login: React.FC<RouteComponentProps> = (props: any) => {
  ...
  const [checkBuyer, setCheckBuyer] = useState('');
  ...
  useEffect(() => {
    props.location.state.data && setCheckBuyer(props.location.state.data);
  ...
  }, [register, countrySelected, props.location.state.data]);
    ...
    const login = async () => {
      ...
      if (result.status === 200) {
        ...
        navigate('/app/verify-phone', {
          state: { data: { ...data, checkBuyer } },
        });
      } else if (result.status === 201) {
        setLoading(false);
        navigate('/app/signup', { state: { data: { ...data, checkBuyer } } });
      } else {
